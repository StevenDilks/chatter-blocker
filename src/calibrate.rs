...
/// Bin gaps and return the low edge of the first zero bucket after a run of
/// non-zero buckets — the valley between the chatter cluster and legit typing.
/// Returns None when there's too little data or no valley was found.
pub fn suggest_from_gaps(gaps: &[u32]) -> Option<u32> {
    ...
    for &g in gaps {
        if g < MAX_MS {
            buckets[(g / BUCKET_MS) as usize] += 1;
        }
    }
    ...
    for (i, &count) in buckets.iter().enumerate() {
        if count > 0 {
            ...
            return Some((i as u32) * BUCKET_MS);
        }
    }
    None
}
...
pub fn format_report(snapshot: &HashMap<u32, Vec<u32>>) -> String {
    ...
    writeln!(out, "[calibrate] report:").unwrap();
    ...
    for vk in keys {
        ...
        let suggest = suggest_from_gaps(gaps)
            .map(|t| format!("{t}ms"))
            .unwrap_or_else(|| "—".into());
        writeln!(
            out,
